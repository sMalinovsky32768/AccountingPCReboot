        ...
        public void CreateReportExcel(string path = null)
        {
            ...
            try
            {
                ...
                for (var i = 0; i < dataSet.Tables.Count; i++)
                {
                    ...
                    for (var j = 0; j < columnCount; j++) worksheet.Cells[1, j + 2] = dataTable.Columns[j].ColumnName;
                    // Заполнение ячеек
                    for (var j = 0; j < rowCount; j++)
                    {
                        var row = dataTable.Rows[j];
                        for (var k = 0; k < columnCount; k++) worksheet.Cells[j + 2, k + 2] = row[k];
                    }
                    ...
                    if (Options.IsShowUnitOfMeasurement)
                    {
                        ...
                        if (cost != null)
                        {
                            ...
                            if (totalCost != null)
                            {
                                ...
                                if (count != null)
                                {
                                    ...
                                    var formula =
                                        $"={newRange.Address[false, false]}*{newRange1.Address[false, false]}";
                        ...
                        dynamic diagonal = worksheet.Cells.Find("Диагональ") ??
                                           (worksheet.Cells.Find("Диагональ экрана") ??
                                            worksheet.Cells.Find("Максимальная диагональ"));
                        if (diagonal != null)
                        {
                            var target = worksheet.Range[
                    ...
                    if (Options.IsCountMaxMinAverageSum)
                    {
                        ...
                        if (cost != null)
                        {
                            ...
                            ((Range) worksheet.Cells[rowCount + 2, ((Range) cost).Column]).Formula =
                                $"=SUM({begin.Address[false, false]}:{end.Address[false, false]})";

                            worksheet.Cells[rowCount + 3, ((Range) cost).Column - 1] = "Минимальная цена";
                            ((Range) worksheet.Cells[rowCount + 3, ((Range) cost).Column]).Formula =
                                $"=MIN({begin.Address[false, false]}:{end.Address[false, false]})";

                            worksheet.Cells[rowCount + 4, ((Range) cost).Column - 1] = "Средняя цена";
                            ((Range) worksheet.Cells[rowCount + 4, ((Range) cost).Column]).Formula =
                                $"=AVERAGE({begin.Address[false, false]}:{end.Address[false, false]})";

                            worksheet.Cells[rowCount + 5, ((Range) cost).Column - 1] = "Максимальная цена";
                            ((Range) worksheet.Cells[rowCount + 5, ((Range) cost).Column]).Formula =
                                $"=MAX({begin.Address[false, false]}:{end.Address[false, false]})";

                            if (Options.IsShowUnitOfMeasurement)
                                for (var j = 2; j <= 5; j++)
                                    ((Range) worksheet.Cells[rowCount + j, ((Range) cost).Column]).NumberFormat =
                                        "# ##0 \u20bd";
                        }
                        ...
                        if (totalCost != null)
                        {
                            ...
                            ((Range) worksheet.Cells[rowCount + 2, ((Range) totalCost).Column]).Formula =
                                $"=SUM({begin.Address[false, false]}:{end.Address[false, false]})";

                            if (Options.IsShowUnitOfMeasurement)
                                ((Range) worksheet.Cells[rowCount + 2, ((Range) totalCost).Column]).NumberFormat =
                                    "# ##0 \u20bd";
                        }
                        ...
                        if (count != null)
                        {
                            ...
                            ((Range) worksheet.Cells[rowCount + 2, ((Range) count).Column]).Formula =
                                $"=SUM({begin.Address[false, false]}:{end.Address[false, false]})";
                        }
                        ...
                        if (inventory != null)
                        {
                            ...
                            ((Range) worksheet.Cells[rowCount + 2, ((Range) inventory).Column]).Formula =
                                $"=COUNT({begin.Address[false, false]}:{end.Address[false, false]})";
                            if (Options.IsShowUnitOfMeasurement)
                                ((Range) worksheet.Cells[rowCount + 2, ((Range) inventory).Column]).NumberFormat =
                                    "0__Устройств";
                        }
                    }

                    var range = worksheet.Range[worksheet.Cells[1, 1], worksheet.Cells[rowCount + 2, columnCount + 2]];
                    range.EntireColumn.AutoFit();
                }

                if (workbook.Worksheets.Count > 1) activeWorksheet.Delete();
                if (path == null)
                {
                ...
                {
                    if (Options.CreateOptions == CreateReportOptions.SaveAsXlsx) workbook.SaveAs(path);
